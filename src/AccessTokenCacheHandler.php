...
class AccessTokenCacheHandler
{
    ...
    public function saveTokenByProvider(AccessToken $accessToken, OAuth2Provider $provider, array $options): bool
    {
        ...
        $expirationOffset = max(0, $options['token_expiration_reduction'] ?? 0);
        //to avoid expiration of token during request
        $expiration = new \DateTime();
        $expiration->setTimestamp($accessToken->getExpires() - $expirationOffset);
        $cacheItem->expiresAt(
            $expiration
        );
